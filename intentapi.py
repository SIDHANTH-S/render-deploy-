...
# Load environment variables (commented out since API key is hardcoded)
# load_dotenv()
...
SONAR_API_TOKEN = "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"

@app.post("/proxy-sonar")
async def proxy_sonar(request: Request):
    if not SONAR_API_TOKEN:
        raise HTTPException(
            status_code=500, 
            detail="SONAR_API_TOKEN not configured. Please set the SONAR_API_TOKEN environment variable."
        )
    ...
    try:
        ...
        if response.status_code == 401:
            raise HTTPException(status_code=401, detail="Invalid API key for Perplexity Sonar")
